...
const RoutineTracker = () => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-0 right-0 z-50 flex justify-evenly p-4 bg-gradient-to-b from-black/40 to-transparent backdrop-blur-sm">
        {musicThemes.map((theme) => (
          <button
            key={theme.id}
            onClick={() => handleMusicClick(theme)}
            className={`text-xl font-black tracking-wide transition-all focus:outline-none transform hover:scale-110 duration-300 px-4 py-2 rounded-2xl ${
      ...
      {/* Themes Sidebar */}
      {!isSidebarMinimized ? (
        <div className="absolute top-4 left-4 bottom-4 w-80 bg-white/10 backdrop-blur-xl border border-white/30 rounded-3xl p-6 shadow-2xl z-50 overflow-y-auto">
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-gradient-to-br from-teal-400 to-lime-500 rounded-xl flex items-center justify-center">
                <span className="text-xl">🎨</span>
              </div>
              <h2 className="text-2xl font-black bg-gradient-to-r from-teal-200 to-lime-200 bg-clip-text text-transparent">Themes</h2>
            </div>
            <button
          ...
          <input
            ...
            className="w-full p-4 bg-white/10 backdrop-blur-sm border border-white/30 text-white placeholder-white/50 rounded-2xl mb-6 focus:outline-none focus:ring-2 focus:ring-teal-400 transition-all font-medium"
          />
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            {localThemes.map((theme) => (
              <button
                ...
                className="w-16 h-16 border-2 border-white/30 rounded-2xl flex items-center justify-center shadow-lg hover:scale-110 transition-transform duration-300 hover:border-teal-400"
            ...
            {youtubeThemes.length > 0 ? (
              youtubeThemes.map((theme) => (
                <button
                  key={theme.id}
                  onClick={() => setSelectedTheme(theme)}
                  className="bg-white/10 backdrop-blur-sm border border-white/30 rounded-2xl p-4 flex flex-col hover:bg-white/20 transition-all duration-300 hover:scale-105 transform"
                >
                  <span className="text-sm font-bold text-white mb-1">{theme.name}</span>
                  <a
                    href={theme.src}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-xs text-teal-300 hover:text-teal-200 mt-1 break-all"
                  >
                    {theme.src}
                  </a>
                </button>
          ...
          {selectedTheme.type === "video" &&
            !selectedTheme.src.includes("youtube.com") && (
              <div className="mt-6 p-4 bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl">
                <label htmlFor="volume" className="block text-sm font-bold text-white mb-3 flex items-center gap-2">
                  <span>🔊</span> Volume
                </label>
                <input
                  type="range"
        ...
        <button
          ...
          className="fixed top-1/2 left-0 transform -translate-y-1/2 z-50 px-5 py-3 bg-gradient-to-r from-teal-400 to-emerald-500 text-white font-black rounded-r-2xl shadow-2xl hover:from-teal-500 hover:to-emerald-600 transition-all duration-300 hover:scale-105 hover:shadow-teal-500/50"
        >
          🎨 Themes
        </button>
      )}
      ...
      {!isCalendarMinimized ? (
        <Rnd
          ...
          style={{ zIndex: 50 }}
        >
          <div className="flex justify-between items-center bg-gradient-to-r from-teal-500 to-emerald-500 p-4 cursor-move rounded-t-3xl">
            <div className="flex items-center gap-2">
              <span className="text-2xl">📅</span>
              <span className="font-black text-white text-lg">Calendar</span>
            </div>
            <button
              onClick={() => setCalendarMinimized(true)}
              className="text-white hover:text-white/70 transition-colors p-2 hover:bg-white/20 rounded-xl"
            >
              {/* Minimize Icon */}
              <svg
                xmlns="http://www.w3.org/2000/svg"
          ...
          <div className="h-full overflow-auto bg-white/10 backdrop-blur-xl rounded-b-3xl shadow-inner border-x border-b border-white/30">
            <DailyCalendar
        ...
        <button
          ...
          className="fixed bottom-24 right-8 z-50 px-6 py-4 bg-gradient-to-r from-teal-400 to-emerald-500 text-white font-black rounded-2xl shadow-2xl hover:from-teal-500 hover:to-emerald-600 transition-all duration-300 hover:scale-110 hover:shadow-teal-500/50"
        >
          📅 Calendar
        </button>
      )}
      ...
      <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 w-full max-w-md px-4 z-10">
        {editingTask ? (
          <form
            ...
            className="flex items-center bg-white/10 backdrop-blur-xl border border-white/30 p-4 rounded-3xl shadow-2xl"
          >
            <input
              type="text"
              value={editTaskText}
              ...
              className="flex-1 p-4 bg-white/10 backdrop-blur-sm text-white placeholder-white/50 border border-white/20 rounded-2xl outline-none focus:ring-2 focus:ring-teal-400 transition-all font-medium"
            />
            <button
              type="submit"
              className="ml-2 px-6 py-4 bg-gradient-to-r from-blue-400 to-blue-500 text-white font-black rounded-2xl hover:from-blue-500 hover:to-blue-600 transition-all duration-300 transform hover:scale-105 shadow-xl"
            >
              ✓ Update
            </button>
            <button
              ...
              className="ml-2 px-6 py-4 bg-gradient-to-r from-gray-400 to-gray-500 text-white font-black rounded-2xl hover:from-gray-500 hover:to-gray-600 transition-all duration-300 transform hover:scale-105 shadow-xl"
            >
              ✕ Cancel
            </button>
          </form>
          ...
          >
            ...
            <button
              type="submit"
              className="ml-4 px-8 py-4 bg-gradient-to-r from-lime-400 to-emerald-500 text-white font-black rounded-2xl hover:from-lime-500 hover:to-emerald-600 transition-all duration-300 transform hover:scale-105 shadow-xl hover:shadow-lime-500/50"
            >
              ➕ Add
            </button>
          </form>
      ...
      {showTaskModal && selectedTask && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={() => setShowTaskModal(false)}>
          <div className="bg-gradient-to-br from-slate-900 to-slate-800 border-2 border-teal-400/50 rounded-3xl p-8 shadow-2xl max-w-md w-full mx-4 transform transition-all" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-2xl font-black bg-gradient-to-r from-teal-200 to-emerald-200 bg-clip-text text-transparent">Manage Task</h3>
            </div>
            
            <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-4 mb-6">
              <p className="text-white font-bold text-lg mb-2">{selectedTask.task}</p>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 px-6 py-4 bg-gradient-to-r from-blue-400 to-blue-500 text-white font-black rounded-2xl hover:from-blue-500 hover:to-blue-600 transition-all duration-300 transform hover:scale-105 shadow-xl flex items-center justify-center gap-2"
              >
              ...
              <button
                ...
                className="flex-1 px-6 py-4 bg-gradient-to-r from-red-400 to-red-500 text-white font-black rounded-2xl hover:from-red-500 hover:to-red-600 transition-all duration-300 transform hover:scale-105 shadow-xl flex items-center justify-center gap-2"
              >
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
            ...
            <button
              ...
              className="mt-4 w-full px-6 py-3 bg-white/10 hover:bg-white/20 text-white font-bold rounded-2xl transition-all duration-300"
            >
              Close
            </button>
          </div>
      ...
      {showConflictModal && conflictingTasks.length > 0 && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={handleCancelConflict}>
          <div className="bg-gradient-to-br from-slate-900 to-slate-800 border-2 border-orange-400/50 rounded-3xl p-8 shadow-2xl max-w-md w-full mx-4 transform transition-all" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-6">
              <div className="w-12 h-12 bg-gradient-to-br from-orange-400 to-red-500 rounded-2xl flex items-center justify-center">
                <span className="text-2xl">⚠️</span>
              </div>
              <h3 className="text-2xl font-black bg-gradient-to-r from-orange-200 to-red-200 bg-clip-text text-transparent">Time Slot Conflict</h3>
            </div>
            
            <p className="text-white/90 mb-4 text-lg font-semibold">
              This time slot is already occupied by {conflictingTasks.length} task{conflictingTasks.length > 1 ? 's' : ''}:
            </p>

            <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-4 mb-6 max-h-48 overflow-y-auto">
              {conflictingTasks.map((task, index) => (
                <div key={task.id} className={`${index > 0 ? 'mt-3 pt-3 border-t border-white/10' : ''}`}>
                  <p className="text-white font-bold text-base mb-1">{task.task}</p>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 px-6 py-4 bg-gradient-to-r from-orange-400 to-red-500 text-white font-black rounded-2xl hover:from-orange-500 hover:to-red-600 transition-all duration-300 transform hover:scale-105 shadow-xl flex items-center justify-center gap-2"
              >
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
              ...
              <button
                ...
                className="flex-1 px-6 py-4 bg-gradient-to-r from-gray-400 to-gray-500 text-white font-black rounded-2xl hover:from-gray-500 hover:to-gray-600 transition-all duration-300 transform hover:scale-105 shadow-xl flex items-center justify-center gap-2"
              >
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
